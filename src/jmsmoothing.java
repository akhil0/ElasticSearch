...
public class jmsmoothing {
	...
	private static void printmap(HashMap<String, Double> resultmap, String string) {
		...
		int id0 = 0;
		File log = new File("C:\\Users\\AKI\\workspace\\Elasticsearch\\jmsmoothing.txt");
	...
	public static HashMap<String, Double> queryTF(Client client, String qb, String index, String type, HashMap<String, Double> result,HashMap<String, Double> lengthmap,HashMap<String, Double> staticmap) throws IOException, JSONException {
		...
		SearchResponse responsesearch = client.prepareSearch(index)
				...
				.add(ScoreFunctionBuilders
						...
						.execute()
						.actionGet();
		JSONObject obj = new JSONObject(responsesearch.toString());
		JSONObject obj2 = obj.getJSONObject("hits");
		double cfw = obj2.getDouble("total");
		...
		for(int i=0 ; i < hits.length() ; i ++)
		{
			...
			Double tf = newobj.getDouble("_score");
			ttf = ttf + tf;
		}
		...
		while(itrnext.hasNext())
		{
			...
			double lambda = len/(avg+len) ;
			double jm = Math.log((lambda*(0/len)) + (1-lambda)*(ttf/(84678*248)));
			//double jm = Math.log(0/(avg+len) + (avg*ttf)/((avg+len)*v));
			staticmap.put(id, oldtf+jm);
		}
		...
		{
			JSONObject newobj = hits.getJSONObject(i);	
			String id = newobj.getString("_id");
			Double tf = newobj.getDouble("_score");
			double len = lengthmap.get(id);
			double avg = 248;
			double v = 178081 ;
			double lambda = len/(avg+len) ;
			double jm = Math.log((lambda*(tf/len)) + (1-lambda)*(ttf/(84678*248)));
			double inc = Math.log((lambda*(0/len)) + (1-lambda)*(ttf/(84678*248)));
			//double jm = Math.log(tf/(avg+len) + (avg*ttf)/((avg+len)*v));
			//double inc = Math.log(0/(avg+len) + (avg*ttf)/((avg+len)*v));

			if(staticmap.containsKey(id))
			{
				double oldtf = staticmap.get(id);
				staticmap.put(id, oldtf + jm - inc);
			}
			else
				staticmap.put(id, jm);
		}
